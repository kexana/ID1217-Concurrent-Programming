...
void* thread_fun (thread_data* data){
    ...
    for (int i = data->start; i < data->end; i++){
        ...
        for (int j = 0; j < reverse.size() / 2; j++)    swap(reverse[j], reverse[reverse.size() - 1 - j]); 
...
int main(int argc, char *argv[]){
    ...
    #pragma omp parallel
    {
        ...
        thrd_data[thread_id].end = (thread_id == workers - 1)? words.size()-1 : (words.size()/workers) * (thread_id+1);
